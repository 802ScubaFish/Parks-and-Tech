...
function facilitySearch() {
    ...
    myDb.ref("/Facilities").once("value", (res) => {
        ...
        // If no filter facilities are selected then we return our unchanged global event variable to re-display the full events list
        if (unsortedFacilitiesArray.length === 0) {
            
          setMapOnAll(map, markersArray)
        }

        let markerUids = markersArray.filter((markerObj => {
            for (let i = 0; i < unsortedFacilitiesArray.length; i++) {

                if (markerObj.uid == unsortedFacilitiesArray[i].uid) {
                    return true
                }
            }
        }))
